...
def main():
    ...
    # Future window: We want to evaluate the max price in the 2.5 hour window that STARTs 30 mins from now.
    ...
    # We also need to know the actual buy price (the price exactly 30 mins from "now")
    future_buy_price = (
        df.select(['isin', 'tradeTime', pl.col('price').alias('buy_price')])
        .with_columns([
    ...
    future_buy_price = future_buy_price.collect().sort(['isin', 'tradeTime']).lazy()
    ...
    future_buy_price = future_buy_price.with_columns(pl.col('tradeTime').set_sorted())
    ...
    final_df = (
        ...
        .with_columns([
            ...
            (pl.col("future_2h30m_max_price") >= (pl.col("past_2h_max_price") * 1.005)).cast(pl.Int32).alias("target_label")
        ])
        .drop_nulls()
        .collect()
    )
    ...
    for i, stock_isin in enumerate(unique_isins):
        ...
        features = ['current_price', 'past_2h_max_price', 'past_2h_volume', 'past_2h_tick_count', 'bench_delta']
        target = 'target_label'
        ...
        try:
            ...
            breakout_freq = test_breakouts / len(y_test) if len(y_test) > 0 else 0

            # Generate trading signals
            trading_signals = (y_pred_prob >= optimal_threshold).astype(int)

            # P&L Calculation:
            # We buy at buy_price (30 min lag) and sell at sell_price (end of window).
